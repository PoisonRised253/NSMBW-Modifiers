...
{
public:
	dStateWrapperBase_c(TOwner *pOwner) : manager(&pointless, &executor, &dStateBase_c::mNoState), executor(pOwner) {}

	dStateWrapperBase_c(TOwner *pOwner, dState_c<TOwner> *pInitState) : manager(&pointless, &executor, pInitState), executor(pOwner) {}
...
{
public:
	dStateWrapper_c(TOwner *pOwner) : dStateWrapperBase_c<TOwner>(pOwner) {}

	dStateWrapper_c(TOwner *pOwner, dState_c<TOwner> *pInitState) : dStateWrapperBase_c<TOwner>(pOwner, pInitState) {}
...
#define CREATE_STATE(CLASS, NAME)                                   \
	CLASS::State CLASS::StateID_##NAME(#CLASS "::StateID_" #NAME,   \
...
class daWMPlayer_c {
	//Note: Dont Workie :()
public:
	void checkWalkPlayers();
	void getSubPlayerNum();
	void *getWalkEffectId(int unk);
	void *getLandingEffectId(int unk);
	void *getCourseInVoiceId(int unk);
	Vec getWipeCenter();
	Vec getWipeCenter(const Vec &);
	void startLastBridgeMotor();
	void startBridgeMotor();
	bool isPlayerStarMode();
	void checkOtherWaitAntlion();
	void startStarSound();
	// setEnemyDieByStar__12daWmPlayer_cFP10dWmEnemy_c
	void attackMapEnemy(bool unk);
	bool isWalkToAttackPoint();

	static daWMPlayer_c *ms_instance;
	static void* smc_MAX_SPEED;
	static void* smc_GRAVITY;
	static void* smc_MAX_FALL_SPEED;
	static void* smc_DOKAN_DEPTH;
	static void* smc_CANNON_DEPTH;
};
...
inline void scaleDown(Vec *scale, float amt)
{
	scale->x -= amt;
	scale->y -= amt;
	scale->z -= amt;
}
inline void scaleUp(Vec *scale, float amt)
{
	scale->x -= amt;
	scale->y -= amt;
	scale->z -= amt;
...
extern "C" void __set_debug_bba(int x); // 0x80004030
